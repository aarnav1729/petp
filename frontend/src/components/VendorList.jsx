...
const VendorList = () => {
  ...
  const fetchFactoryUsers = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/api/factory-users"
      );
  ...
  const addVendor = async (e) => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/api/add-vendor",
        vendorFormData
      );
      fetchVendors();
      setVendorFormData({
  ...
  const addFactoryUser = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://localhost:5000/api/add-factory-user", factoryFormData);
      fetchFactoryUsers();
  ...
  const deleteFactoryUser = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/factory-users/${id}`);
      fetchFactoryUsers();
    } catch (error) {
