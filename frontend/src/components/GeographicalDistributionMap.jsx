// GeographicalDistributionMap.jsx
...
const GeographicalDistributionMap = () => {
  ...
  useLayoutEffect(() => {
    let chart = am4core.create(
      "geographicalDistributionMapDiv",
      am4maps.MapChart
    );
    ...
    polygonTemplate.tooltipText = "{name}: {value} shipments";
    ...
    // Create heat rule with new colors
    polygonSeries.heatRules.push({
      property: "fill",
      target: polygonSeries.mapPolygons.template,
      min: am4core.color("#F5E0C7"), // Light peach
      max: am4core.color("#B22222"), // Firebrick
    });
    ...
    heatLegend.marginTop = 430; // Significant padding
    heatLegend.marginBottom = 0; // Add bottom margin to prevent overlap

    // Adjust chart's bottom padding to accommodate the legend
    chart.chartContainer.paddingBottom = 70; // Increased padding
    ...
    let data = mapData.map((item) => ({
      id: getStateIdByName(item.state),
      name: item.state,
      value: item.count,
    }));
    ...
    chart.homeGeoPoint = { latitude: 23.0, longitude: 82.0 };
  ...
  function getStateIdByName(stateName) {
    // Map state names to their corresponding id in am4geodata_india2019High
    const stateIdMap = {
  ...
  function fetchDistrictData(stateName) {
    ...
    axios
      ...
      .then((response) => {
        // Optionally sort the data by shipment count descending
  ...
  return (
    <div className="map-container">
      <div id="geographicalDistributionMapDiv" className="map-div"></div>
      {selectedState && (
        <div className="district-breakdown">
          <h3>Shipments to Districts in {selectedState}</h3>
          <button
            onClick={() => {
