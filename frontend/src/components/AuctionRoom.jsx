...
const AuctionRoom = ({ username, role }) => {
  ...
  useEffect(() => {
    const fetchRFQDetails = async () => {
      try {
        const response = await axios.get(`https://leaf.premierenergiesphotovoltaic.com/api/rfq/${rfqId}`);
        ...
        if (role === "vendor") {
          ...
          const bidResponse = await axios.get(`https://leaf.premierenergiesphotovoltaic.com/api/quotes/${rfqId}`);
  ...
  useEffect(() => {
    const fetchBids = async () => {
      try {
        const response = await axios.get(`https://leaf.premierenergiesphotovoltaic.com/api/quotes/${rfqId}`);
        if (role === "vendor") {
  ...
  const handlePlaceBid = async () => {
    ...
    if (!isNaN(price) && !isNaN(numberOfTrucks)) {
      try {
        ...
        const rfqResponse = await axios.get(`https://leaf.premierenergiesphotovoltaic.com/api/rfq/${rfqId}`);
        ...
        if (vendorQuotes[rfqId]) {
          // Update existing bid
          await axios.put(
            `https://leaf.premierenergiesphotovoltaic.com/api/quote/${vendorQuotes[rfqId]._id}`,
            {
              rfqId,
              vendorName,
              quote: price,
          ...
          // Create a new bid
          await axios.post("https://leaf.premierenergiesphotovoltaic.com/api/quote", {
            rfqId,
            vendorName,
            quote: price,
            numberOfTrucks,
